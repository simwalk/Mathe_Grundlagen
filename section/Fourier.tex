	...
	\subsection{Sätze zur Berechnung der Fourierkoeffizienten}
		...
		\subsection{Komplexe Darstellung der Fourierreihen (in $\mathbb{C}$)}
			\begin{minipage}[t]{0.5\textwidth}
				...
				\fbox{$\sum\limits_{k=-\infty}^{\infty} c_k \cdot \mathrm{e}^{\mathrm{j} k \omega_f t}$}\\[3pt]
				\textbf{Umrechungsformeln ($a_n$, $b_n \rightarrow c_n$):}\\[3pt]
				\fbox{$c_n = \dfrac{a_n - \mathrm{j} b_n}{2}$} für $n = 0, 1, 2, 3, \cdots (b_0 = 0)$\\[3pt]
				\fbox{$c_{-n} = \dfrac{a_n + \mathrm{j} b_n}{2} = \overline{c_n}$} für $n = 1, 2, 3, \cdots (b_0 = 0)$\\[3pt]
			\end{minipage}
			\begin{minipage}[t]{0.5\textwidth}
				\textbf{Komplexe Fourierkoeffizienten:}\\[3pt]
				\fbox{$c_n = \overline{c_{-n}} = \dfrac{1}{T} \int\limits_{0}^{T} f(t) \cdot \mathrm{e}^(\mathrm{j} n \omega_f t) dt$} für $n = 0, 1, 2, 3, \cdots$\\[3pt]
